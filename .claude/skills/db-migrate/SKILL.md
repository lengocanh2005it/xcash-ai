---
name: db-migrate
description: Quy trình chuẩn khi thêm/sửa bảng trong Prisma schema của Klassi AI — đối chiếu database-schema.md, tạo migration, cập nhật shared-types và index bắt buộc. Dùng khi cần đổi schema DB hoặc khi user gõ "/db-migrate".
---

# DB Migrate — Quy trình sửa Prisma schema chuẩn Klassi AI
...
# 2. Tạo migration (dev)
pnpm --filter @klassi/backend exec prisma migrate dev --name <mo_ta_ngan_gon>

# 3. Generate lại Prisma Client
pnpm --filter @klassi/backend exec prisma generate

# 4. Type-check để bắt lỗi chỗ code cũ dùng field/model đã đổi
pnpm --filter @klassi/backend type-check
```

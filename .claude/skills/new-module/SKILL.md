---
name: new-module
description: Scaffold một NestJS module mới trong apps/backend/src/modules/ đúng chuẩn Klassi AI (controller, service, DTO, RBAC guard, tenant scoping). Dùng khi cần thêm domain module mới (vd billing, partner, settings...) hoặc khi user gõ "/new-module <tên>".
---

# New Module — Scaffold NestJS module chuẩn Klassi AI
...
2. **`<domain>.controller.ts`** phải có:
   ...
   - Import `Role` từ `@klassi/shared-types`, không tự định nghĩa enum role riêng trong module.
...
5. Response trả về đúng format `ApiResponse<T>` (từ `@klassi/shared-types`) qua interceptor toàn cục — controller chỉ return data thô, không tự bọc `{ success, data, ... }` bằng tay.

## Sau khi scaffold xong

- Chạy `pnpm --filter @klassi/backend type-check` và `pnpm --filter @klassi/backend lint` để bắt lỗi sớm.

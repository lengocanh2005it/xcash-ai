---
name: verify
description: Chạy bộ kiểm tra đầy đủ (lint, type-check, test, build) trên toàn monorepo Klassi AI sau khi sửa code. BẮT BUỘC dùng skill này trước khi báo cáo task hoàn thành, hoặc khi user gõ "/verify".
---
...
Đây là alias cho `turbo run lint type-check test build` — chạy cả 4 bước, Turborepo tự xử lý thứ tự phụ thuộc giữa các package (`@klassi/shared-types` build/check trước `@klassi/backend`/`@klassi/frontend` nếu có phụ thuộc) và cache kết quả không đổi.

## Nếu chỉ sửa 1 package, có thể verify nhanh hơn bằng `--filter`

```bash
pnpm exec turbo run lint type-check test build --filter=@klassi/backend
pnpm exec turbo run lint type-check test build --filter=@klassi/frontend
```
...
- **`build`** — lỗi build thường là do thiếu export, sai path import giữa package (`@klassi/shared-types` chưa `pnpm install` lại sau khi thêm dependency mới — xem `agent-docs/01-monorepo-structure.md` mục "Cách package nội bộ import lẫn nhau").

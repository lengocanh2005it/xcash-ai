...
Cập nhật lần cuối: **Copilot export báo cáo (Excel/PDF)** — thêm tool `export_report` (registry): nhận `format` (excel/pdf) + `year+month` HOẶC `startDate+endDate`, validate mutually-exclusive; `ReportExportService` mới (`apps/backend/src/modules/report/report-export.service.ts`) sinh file (tái dùng `fetchExportData()` + `buildExportWorkbook()` cho Excel, `report-pdf.util.ts` mới dùng `pdfmake`/Roboto font cho PDF), lưu buffer tạm Redis TTL 600s (`copilot:export:{exportId}`), tool trả `{exportId, format, fileName, fromDate, toDate}` (không lộ buffer qua model). Endpoint mới `GET /reports/copilot-export/:exportId` — Redis hit trả ngay, miss → regenerate on-demand từ `fromDate/toDate` (validate thiếu tham số → 400), 403 nếu sai tenant. Activity kind mới `file_export` (shared-types `CopilotFileExportData`) lưu vĩnh viễn trong copilot history — FE (`CopilotFileExportCard.tsx`) hiển thị nút "Tải về" trong khung chat, tải qua blob fetch có auth (không dùng href thường). `pnpm verify` pass 11/11.

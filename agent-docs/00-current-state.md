...
Cập nhật lần cuối: **useAuditLog extraction (candidate #5, hoàn tất) + candidate #7 khảo sát (không hành động — premise không đủ căn cứ)** — Candidate #5: tách `AuditLogPanel.tsx` (396 dòng, 8 state field: page/searchInput/actionFilter/entityTypeFilter/fromDate/toDate/selectedItem/sheetOpen) thành `hooks/useAuditLog.ts`, tái dùng thẳng toolkit `useFilteredPagination` đã có sẵn (cùng pattern `usePartnerTenants.ts`) thay vì tự quản lý debounce/pagination riêng; hook sở hữu luôn state detail-sheet (`selectedItem`/`sheetOpen`/`openDetail`); `AuditLogItem` type dời từ `AuditLogPanel.tsx` sang `useAuditLog.ts`, `AuditLogDetailSheet.tsx` cập nhật import; `AuditLogPanel.tsx` giờ chỉ còn render + 2 mảng option tĩnh (ACTION_FILTER_OPTIONS/ENTITY_TYPE_OPTIONS ở lại component vì là dữ liệu trình bày). Candidate #7 ("dedupe edit-in-place + confirm dialog pattern" giữa `PartnerPlansPage`/`PartnerTenantsPage`): khảo sát cho thấy `PartnerTenantsPage`'s set-plan dialog là single-select dropdown (state đã sống trong `usePartnerTenants.ts`), khác hẳn shape 5-field form của `PartnerPlansPage` — chỉ có **1** call site thật sự khớp shape bị flag, không phải 2 như review gốc phỏng đoán (review gốc đã tự đánh dấu "Speculative" vì lý do này) → không xây abstraction, giữ nguyên `PartnerPlansPage.tsx`. `pnpm verify` pass 11/11.

Trước đó — **OtpFlowService extraction (architecture review candidate #3 — premise mở rộng: không phải "Redis key registry" mà là OTP state-machine bị nhân 3)** — điều tra cho thấy `email-verification.service.ts` và `password-reset.service.ts` **giống hệt nhau byte-for-byte** (chỉ khác prefix key + tên field payload email job); `change-password.service.ts` cùng cơ chế cộng thêm `resend()`. Cả 3 đọc chung 3 env var (`EMAIL_OTP_TTL_SECONDS`/`EMAIL_OTP_RESEND_COOLDOWN_SECONDS`/`EMAIL_OTP_MAX_ATTEMPTS`) và tự cài lại logic generate/store/verify/cooldown/attempt-tracking ~100 dòng mỗi file. Tạo `common/services/otp-flow.service.ts` — `OtpFlowService` generic `send<T>(prefix, identifier, payload): {otp, ttlSeconds}` / `verify<T>(prefix, identifier, code): T` / `peek<T>(prefix, identifier): T|undefined`; parse TTL/cooldown/maxAttempts 1 lần; giữ nguyên format key cũ (`${prefix}_otp:${id}` / `${prefix}_resend:${id}`) — không đổi format, an toàn khi deploy giữa chừng có OTP đang hiệu lực; đăng ký trong `@Global() CommonServicesModule` (cùng pattern `CopilotQuotaManager`). Refactor cả 3 service compose qua `OtpFlowService`: `email-verification`/`password-reset` giờ chỉ còn `sendOtp`/`verifyOtp` mỏng; `change-password` giữ `resend()` riêng (đọc payload cũ qua `peek()` vì caller chỉ có `userId`, không có `email`/`newPasswordHash` lúc resend) + check ownership `payload.userId !== userId` sau `verify()`. Thêm `otp-flow.service.spec.ts` test trực tiếp state machine (cooldown 429, tăng attempts giữ nguyên TTL còn lại, xóa key khi hết attempts, không resave khi TTL đã hết); sửa `change-password.service.spec.ts` mock `OtpFlowService` thay vì `RedisService`/`ConfigService` trực tiếp. `pnpm verify` pass 11/11, 41 suites, 292 tests.

Trước đó — **usePartnerDashboardData extraction (architecture review candidate #4 — hoàn tất)** — tách `PartnerDashboardPage.tsx` (422 dòng, 4 `useQuery` inline + 4 `useMemo` derive từ `tenants`) thành hook `hooks/usePartnerDashboardData.ts`: hook sở hữu state `fromDate`/`toDate`/`hasDateFilter`/`clearDateFilter`, cả 4 query (`stats`, `tenants`, `revenueTrend`, `aiCosts` — giữ nguyên 4 loading flag riêng, KHÔNG gộp thành 1 `isLoading` vì mỗi card/chart phụ thuộc query khác nhau), và 4 giá trị derive (`planDistribution`, `planBreakdown`, `mrr`, `topRevenue`). 4 phép tính derive được tách thành pure function export riêng (`computePlanDistribution`, `computePlanBreakdown`, `computeMrr`, `computeTopRevenue`) để test trực tiếp không cần render hook (đúng convention repo — xem `useReportsData.spec.ts`/`useReviewQueue.spec.ts`, chỉ test pure logic export, không dùng `renderHook`). `PartnerDashboardPage.tsx` giờ chỉ còn render (giữ `useNavigate` + JSX date-filter input vì là router/presentation concern). Thêm `usePartnerDashboardData.spec.ts` (11 test): MRR chỉ tính tenant active, top-5 sort/truncate tên >15 ký tự, phần trăm làm tròn, plan `null` → coi là `free`. `pnpm verify` pass 11/11.

Trước đó — **CopilotTransactionQueryService dead-code removal (architecture review candidate #2 — kết luận: premise sai, thu hẹp phạm vi)** — điều tra cho thấy `copilot-tx-query.service.ts` (386 dòng, được liệt kê trong review là "rủi ro cao nhất vì LLM-exposed") **hầu như toàn bộ là dead code**: chỉ `getBankingStatus()` thực sự được gọi qua `ToolDeps`; 7 method còn lại (`searchTransactions`, `getReviewQueueCount`/`listReviewQueue`/`reviewQueueWhere`, `lookupChartAccount`/`listChartAccounts`, `proposeConfirmTransactionClassification`/`proposeCorrectTransactionClassification`) đều có bản song sinh **đang thực sự chạy** ở nơi khác (`transaction.service.searchForCopilot`, `classification.service.getCopilotReviewQueueCount`/`listCopilotReviewQueue`/`proposeConfirmClassification`/`proposeCorrectClassification`, `chart-of-accounts.service.findByCode`/`listFiltered`) — tool registry (`review-tools.ts`, `account-tools.ts`, `action-tools.ts`) trỏ vào các bản đang chạy đó, không phải vào `CopilotTransactionQueryService`. Đã xóa 7 method dead, đổi tên file/class thành `copilot-banking-status.service.ts` / `CopilotBankingStatusService` (chỉ còn `getBankingStatus`, cache Redis 60s); đổi tên field `ToolDeps.copilotTxQueryService` → `bankingStatusService` (sửa `copilot-tool.types.ts`, `copilot-tool-deps.provider.ts`, `account-tools.ts`, `copilot-tool.executor.spec.ts`, `eval-copilot-tool-routing.ts`, `copilot.module.ts`). **Phần "centralize transaction query building" trong đề xuất gốc bị hủy** — sau khi bỏ dead code chỉ còn 2 call site sống (`billing.getCycleTransactions` lọc theo `createdAt`, `transaction.searchForCopilot` lọc theo `source`/`accountCode`/`classificationStatus`) với shape filter khác nhau thật sự — chưa đủ để biện minh cho 1 query builder chung (theo deletion test: xóa 1 trong 2 hàm này chỉ di chuyển logic filter, không cô đọng duplicate thật). `pnpm verify` pass 11/11, 40 suites, 282 tests.

Trước đó — **CopilotStreamService runTurn() collapse (architecture review candidate #1, PR 2/2 — hoàn tất)** — `chat()` (JSON) và `streamChat()` (SSE) giờ là 2 adapter mỏng bọc quanh 1 `private async *runTurn(): AsyncGenerator<TurnEvent>` duy nhất, sở hữu toàn bộ vòng đời 1 turn: setup hội thoại → gọi LLM (có/không tool) → persist (save message + auto-title + quota) → yield `done`. `TurnEvent` = union `activity | delta | done | partial`, mirror đúng tên event SSE cũ (không thêm khái niệm mới). Abort dùng `AbortSignal` chuẩn thay vì callback Express — `streamChat()` chuyển `req.on('close')` thành `controller.abort()` tại HTTP edge; `runTurn()`/`runWithTools()` chỉ biết `AbortSignal`, không biết Express. `TurnEventQueue` (class nội bộ) bridge event push-based từ `CopilotAgentHarness` (EventEmitter) sang generator pull-based. **Behavior unification có chủ đích** (đã trao đổi với user): trước đây `chat()` trả canned error message khi mọi LLM adapter lỗi còn `streamChat()` fallback về `chatProvider.chatCopilot()` thật — giờ cả 2 đều dùng đường fallback thật của `streamChat()`; và trước đây chỉ `chat()` xóa dangling user message khi lỗi toàn bộ (kể cả fallback) — giờ cả 2 đều xóa (qua outer `catch` của `runTurn()`). **Gotcha đáng nhớ khi debug:** `AbortSignal.addEventListener('abort', ...)` đăng ký muộn (sau vài lượt microtask do generator delegation `yield*`) sẽ bỏ lỡ sự kiện abort đã bắn trước đó — phải check `signal.aborted` đồng bộ trước khi addEventListener. `copilot-stream.service.spec.ts` (8 test từ PR 1) pass nguyên vẹn làm characterization an toàn cho refactor này — chỉ cần sửa 1 test fake (mock `abort()`/`finalContent()` phải wire độc lập thời điểm gọi) + đổi `queueMicrotask` → `setImmediate` (mô phỏng đúng timing thật của event loop hơn). `pnpm verify` pass 11/11, 40 suites, 282 tests.

Trước đó — **CopilotToolDepsProvider extraction (architecture review candidate #1, PR 1/2)** — tách 8 dependency chỉ dùng để build `ToolDeps` (`reportService`, `exportService`, `classificationService`, `chartOfAccountsService`, `transactionService`, `copilotTxQueryService`, `knowledgeService`, `billingService`) ra `copilot-tool-deps.provider.ts`; `CopilotStreamService` constructor giảm từ 15 xuống 8 deps (7 orchestration collaborator + 1 `CopilotToolDepsProvider`), đăng ký trong `copilot.module.ts`. `pnpm verify` pass 11/11, 40 suites, 282 tests.

Trước đó — **PaginatedListView adoption** — Refactored 4 pages (PartnerTenantsPage, PartnerPaymentsPage, PartnerAiCostsPage, CopilotHistoryTab) to use `PaginatedListView`, eliminating ~200 lines of duplicated loading/error/empty/dual-layout/pagination boilerplate. `PaginatedListView` is code-split into its own shared chunk (1.62 kB). Skipped 4 pages (ReportsPage, ReviewPage, TransactionsPage, AccountsPage) with custom structures that don't fit the generic wrapper. `pnpm verify` pass 11/11.
...
│   │           ├── report/                    # Tổng hợp + export Excel/PDF + so sánh tháng + top accounts ✅
│   │           │   ├── report.controller.ts   # injects ReportDataService + ReportExportService; + GET copilot-export/:exportId ✅
│   │           │   ├── report-data.service.ts # getComparison(), getTopAccounts(), getSummary(), exportExcel(), fetchExportData() — main business logic ✅
│   │           │   ├── report-export.service.ts # ReportExportService: createExport()/getExportFile() — Redis cache TTL 600s + regenerate on-demand khi miss ✅
│   │           │   ├── report-pdf.util.ts     # buildExportPdf() — pdfmake + Roboto (vfs_fonts) cho tiếng Việt ✅
│   │           │   ├── dto/copilot-export-query.dto.ts # format/fromDate/toDate optional, dùng cho fallback khi Redis miss ✅
│   │           │   ├── report.module.ts       # exports ReportDataService, ReportExportService
...
│           │   │   ├── CopilotSourceChips.tsx    # Chip nguồn tham khảo (internal_data/knowledge/web_search); kind action_card/file_export có icon riêng nhưng render qua card riêng, không qua chip này ✅
│           │   │   ├── CopilotFileExportCard.tsx # Card nút "Tải về" cho activity kind file_export — blob fetch có auth (không dùng <a href>), Excel/PDF icon theo format ✅
...
│   ├── copilot.ts            # CopilotActivity (kind thêm 'file_export'), CopilotConversationSummary, CopilotActionCardData, CopilotFileExportData ✅
...
| GET | `/reports/copilot-export/:exportId` | Bearer JWT | Tải file export (Excel/PDF) tạo bởi tool Copilot `export_report`; query `format/fromDate/toDate` dùng làm fallback khi Redis cache (TTL 600s) đã hết hạn — không có `@RequiresPlan` (đồng nhất với tool không bị plan-gate) |
...
**Dependencies đáng chú ý:** `xlsx` (export Excel), `pdfmake` (export PDF Copilot — dùng Node `PdfPrinter` class API + Roboto font từ `pdfmake/build/vfs_fonts`, ambient types tự viết tại `apps/backend/src/types/pdfmake.d.ts` vì `@types/pdfmake` chỉ mô tả browser bundle API), `openai`, `bullmq`, `@nestjs/bullmq`, `@nestjs/throttler`, `@nestjs/schedule`, `@payos/node`, `resend`, `@prisma/client ^6`, `@xcash/shared-types`.
...
- **Tool `export_report` (Copilot export Excel/PDF):** OpenAI function-calling chỉ truyền JSON qua model, không trả được buffer nhị phân — nên tool KHÔNG trả file, chỉ trả `{exportId, format, fileName, fromDate, toDate}`. File thật (Excel qua `buildExportWorkbook()` có sẵn, PDF qua `report-pdf.util.ts` mới dùng `pdfmake`) lưu buffer base64 vào Redis (`copilot:export:{exportId}`, TTL 600s, kèm `tenantId` để check quyền). Model không biết validate mutually-exclusive `year+month` XOR `startDate+endDate` — validate trong `execute` của tool, throw `BadRequestException` rõ ràng để model tự hỏi lại user. Endpoint tải `GET /reports/copilot-export/:exportId` **không** có `@RequiresPlan` (tool cũng không bị plan-gate khi gọi qua Copilot — pattern chung: mọi tool copilot gọi thẳng service, bỏ qua HTTP guard tầng controller, xem note kế bên) — khi Redis miss (hết TTL hoặc user refresh trang lâu), endpoint **regenerate lại từ `fromDate/toDate`** thay vì lưu file vĩnh viễn (S3...) — lý do: tránh thêm storage mới, tránh rủi ro file cũ lệch số liệu nếu giao dịch bị sửa sau khi export, chi phí tính lại rẻ (vài query aggregation). Nếu Redis miss mà thiếu `fromDate/toDate` trong query fallback → `BadRequestException` (không để `new Date('')` rơi xuống Prisma gây 500). Activity kind mới `file_export` (không phải discriminated-union style của `action_card` — chỉ 1 tool duy nhất nên field đơn `CopilotFileExportData`), dedup trong `buildActivities()` theo `exportId` (không theo tên tool như `ACTION_CARD_TOOLS`). **Lưu ý:** `resultsCapture` (trong `openai.service.ts`) là `Map` keyed theo **tên tool**, nên nếu model gọi `export_report` 2 lần trong cùng 1 turn (vd "xuất Excel tháng 6 và PDF tháng 7"), lần gọi sau sẽ ghi đè kết quả lần trước — dedup theo `exportId` chỉ có tác dụng khi mở rộng `resultsCapture` để key theo từng lần gọi thay vì theo tên tool; hiện tại case 1-export/turn (phổ biến nhất) hoạt động đúng.
- **Pattern chung: tool Copilot gọi thẳng service, bỏ qua HTTP guard/plan-gate của controller tương ứng.** Vd `get_month_comparison` gọi thẳng `ReportDataService.getComparison()` dù `GET /reports/comparison` có `@RequiresPlan('starter')` — plan-gate chỉ áp dụng khi gọi qua REST trực tiếp, không áp dụng khi model gọi qua tool-calling. Đây là hành vi đã tồn tại từ trước (không phải bug mới), cần biết khi thêm tool mới có REST endpoint tương ứng bị gate — quyết định có nên đồng bộ hay chấp nhận khác biệt tùy từng trường hợp.

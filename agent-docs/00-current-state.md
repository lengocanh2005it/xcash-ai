...
Cập nhật lần cuối: **feat/excel-import** — Excel Import (Phase 1–3) hoàn tất: import module backend, dialog 3 bước FE, source filter, usageBreakdown billing. Fix RolesGuard bypass @Public() (SSE stream 401). **Còn lại:** deploy VPS + SSL/nginx, E2E QA thủ công, test Slack/Resend với key thật trên staging.
...
│           │   ├── transactions/              # TransactionsPage (filter source, nút "Nhập từ Excel", badge amber "Import Excel", checkbox bulk-reclassify) + TransactionDetailSheet + ImportTransactionsDialog ✅

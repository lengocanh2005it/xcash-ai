...
- **pnpm workspaces** quản lý dependency giữa các package nội bộ (`@paypilot/backend`, `@paypilot/frontend`, `@paypilot/shared-types`) bằng symlink, không duplicate `node_modules`.
...
paypilot-ai/
├── apps/
│   ├── backend/                 # NestJS — @paypilot/backend
│   │   ├── src/
│   │   │   ├── main.ts
│   │   │   ├── app.module.ts
...
│   │   └── package.json
│   │
│   └── frontend/                # React + Vite — @paypilot/frontend
│       ├── src/
│       │   ├── main.tsx
│       │   ├── App.tsx
...
│
├── packages/
│   └── shared-types/             # @paypilot/shared-types — enum & type dùng chung BE/FE
│       ├── src/index.ts
│       └── package.json
│
...
Mọi thư mục mới muốn tham gia workspace (BE, FE, package dùng chung) phải nằm dưới `apps/` hoặc `packages/` và có `package.json` với field `name` dạng `@paypilot/<tên>`.
...
- `dependsOn: ["^build"]` nghĩa là: trước khi build/lint/test 1 package, Turborepo build trước các package nó phụ thuộc (vd: build `@paypilot/backend` sẽ build `@paypilot/shared-types` trước nếu backend import nó).
- `dev` không cache và chạy liên tục (`persistent: true`) — đúng bản chất dev server.
- Nếu 1 package không có script tương ứng trong `package.json` (vd: `@paypilot/shared-types` chưa có `dev`), Turborepo tự bỏ qua package đó cho task đó — không lỗi.

## Package hiện có

| Package | Đường dẫn | Vai trò |
|---|---|---|
| `@paypilot/backend` | `apps/backend` | NestJS API — xem [`02-backend-conventions.md`](./02-backend-conventions.md) |
| `@paypilot/frontend` | `apps/frontend` | React SPA (Vite) — xem [`03-frontend-conventions.md`](./03-frontend-conventions.md) |
| `@paypilot/shared-types` | `packages/shared-types` | Enum (`Role`, `TransactionStatus`, `InvoiceStatus`...) + type response dùng chung, tránh lệch định nghĩa giữa BE và FE |
...
pnpm dev:backend                          # chỉ BE (--filter=@paypilot/backend)
pnpm dev:frontend                         # chỉ FE (--filter=@paypilot/frontend)
...
pnpm --filter @paypilot/backend exec <command>       # chạy lệnh bất kỳ trong context backend
```
...
`@paypilot/backend` hoặc `@paypilot/frontend` muốn dùng `@paypilot/shared-types`:

1. Thêm vào `dependencies` trong `package.json` của app đó: `"@paypilot/shared-types": "workspace:*"`
2. Chạy `pnpm install` lại ở root để pnpm tạo symlink.
3. Import bình thường: `import { Role, TransactionStatus } from '@paypilot/shared-types'`

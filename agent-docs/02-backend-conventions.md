...
## Danh sách module cần có (theo `reference/business-overview.md`)

```
src/modules/
├── auth/            # register, login, refresh, logout — JWT, KHÔNG qua Cas
├── onboarding/       # Cas Link Grant/Exchange flow
├── banking/          # webhook Cas Balance Hook, cas_grants
├── transaction/      # CRUD giao dịch
├── invoice/           # CRUD hóa đơn, sinh QR VietQR, import Excel
├── customer/          # CRUD khách hàng, import Excel
├── review/             # Human Review queue
├── ai/                # AI Matching pipeline, Copilot, Explain, RAG Knowledge Base
├── analytics/          # báo cáo, export
├── notification/       # Email/Slack/Discord
├── audit-log/          # ghi log toàn hệ thống
├── settings/            # notification config, matching threshold, banking accounts
├── team/                # invite, role management (chỉ Admin)
├── billing/             # subscriptions, quota, PayOS upgrade flow
└── partner/              # route /partner/*, chỉ Cas Partner — xem lưu ý bảo mật bên dưới
```
...
└── enums/                          # KHÔNG tự định nghĩa lại — import từ @paypilot/shared-types
```
...
| Cách tính quota, flow PayOS billing | [`reference/rbac.md`](./reference/rbac.md#-pricing-model--paypilot-tính-phí-doanh-nghiệp-như-thế-nào) |
...
5. Lưu transaction
6. transaction_used_this_cycle += 1 NGAY (không đợi AI Matching)
7. Chạy AI Matching Pipeline
```
...
Dùng type `ApiResponse<T>` từ `@paypilot/shared-types`, bọc qua 1 global `ResponseInterceptor` — không tự format tay trong từng controller.
...
- Confidence threshold mặc định 95, đọc từ `tenants.matching_threshold` (per-tenant, không hardcode).
- **Circuit Breaker:** khi OpenAI API chậm/lỗi, fallback về rule-based matching (so khớp mã hóa đơn trong content + số tiền) thay vì để request treo — xem `reference/business-overview.md` mục Reliability & Observability.
...
- Unit test theo chuẩn NestJS/Jest, đặt cạnh file test (`*.spec.ts`), `pnpm --filter @paypilot/backend test`.

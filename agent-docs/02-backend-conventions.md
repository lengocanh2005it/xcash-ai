...
└── enums/                          # KHÔNG tự định nghĩa lại — import từ @klassi/shared-types
```
...
Dùng type `ApiResponse<T>` từ `@klassi/shared-types`, bọc qua 1 global `ResponseInterceptor` — không tự format tay trong từng controller.
...
- Unit test theo chuẩn NestJS/Jest, đặt cạnh file test (`*.spec.ts`), `pnpm --filter @klassi/backend test`.

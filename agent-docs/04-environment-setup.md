...
| Copilot | `COPILOT_CONTEXT_CACHE_TTL_SECONDS=300`, `COPILOT_AGENT_MAX_CONTEXT_TOKENS=8000` (max tokens cho context window), `COPILOT_CASSO_SEARCH_ENABLED=0` (Tavily web search casso.vn, cần `TAVILY_API_KEY`), `COPILOT_ACTION_TOOLS_ENABLED=0` (bật tool `propose_confirm_transaction_classification` — Copilot đề xuất xác nhận giao dịch inline trong chat) | Agent loop always active; 2 flag cuối default tắt, bật riêng khi cần test/dùng tính năng tương ứng |

...
| Copilot | `COPILOT_USE_FUNCTION_CALLING=1` (0=prompt stuffing cũ, 1=agent loop qua `CopilotAgentHarness`), `COPILOT_CONTEXT_CACHE_TTL_SECONDS=300`, `COPILOT_CASSO_SEARCH_ENABLED=0` (Tavily web search casso.vn, cần `TAVILY_API_KEY`), `COPILOT_ACTION_TOOLS_ENABLED=0` (bật tool `propose_confirm_transaction_classification` — Copilot đề xuất xác nhận giao dịch inline trong chat) | set `COPILOT_USE_FUNCTION_CALLING=1` để bật tool-calling; 2 flag còn lại đều default tắt, bật riêng khi cần test/dùng tính năng tương ứng |
...
| Error Tracking | `SENTRY_DSN` | Sentry DSN để track exceptions production (optional) |

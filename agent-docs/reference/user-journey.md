# 🧭 Klassi AI — User Journey (Hành trình người dùng thực tế)

> Tài liệu này mô tả toàn bộ hành trình của một doanh nghiệp SME từ lúc đăng ký Klassi AI cho đến khi sử dụng hàng ngày — giúp hình dung rõ vai trò của từng bên (Klassi AI, Cas, kế toán SME) trong hệ thống.

**Khác biệt so với Klassi:** Giai đoạn 3 (Sử dụng hàng ngày) đã thay đổi hoàn toàn — AI không còn "ghép giao dịch với hóa đơn" mà thay bằng "định khoản tự động theo TT133". Kế toán không cần nhập hóa đơn, không quản lý khách hàng — chỉ cần xác nhận định khoản AI gợi ý và export Excel cuối tháng.
...
(Klassi AI tự       (liên kết NH        kế toán review)       Free, qua
quản lý)            qua Cas Link)                             PayOS billing)
```

---

## Giai đoạn 1 — Đăng ký & Đăng nhập Klassi AI

Đây là phần **Klassi AI tự quản lý hoàn toàn**, không phụ thuộc Cas.

```
Anh A — chủ trung tâm Anh ngữ ABC, nghe nói đến Klassi AI
        │
        ▼
Vào web Klassi AI → bấm "Đăng ký"
        │
        ▼
...
Anh A ở màn hình Onboarding, bấm "Liên kết tài khoản ngân hàng"
        │
        ▼
Klassi AI Backend gọi POST /grant/token
(dùng x-client-id / x-secret-key của Klassi AI, lấy từ Cas Console)
        │
        ▼
...
Cas Link trả về publicToken qua redirectUri
        │
        ▼
Klassi AI Backend gọi POST /grant/exchange
...
Sau bước này, kế toán **không cần làm gì thêm** ở phía Cas nữa. Webhook nhận giao dịch là **1 URL chung cho toàn bộ App Klassi AI** (đã được team Klassi AI cấu hình sẵn từ trước trên Cas Developer Console) — hệ thống tự nhận diện đúng doanh nghiệp nhờ `grantId` được lưu lại lúc liên kết ngân hàng.

**Thêm ở Klassi AI:** Sau khi liên kết tài khoản ngân hàng, hệ thống **tự động seed danh mục tài khoản TT133** (~60 tài khoản) cho tenant. Kế toán có thể thêm tài khoản con ngay sau đó.

---

## Giai đoạn 3 — Sử dụng hàng ngày

Đây là phần cốt lõi của Klassi AI — AI định khoản tự động, kế toán chỉ cần xác nhận.
...
về URL chung của App (kèm grantId)
        │
        ▼
Klassi AI tra grantId → xác định đúng tenant
        │
        ▼
Klassi AI chạy AI Classification Pipeline:
  → Text preprocessing
...
### So sánh trước và sau khi dùng Klassi AI

| | Trước (thủ công) | Sau (dùng Klassi AI) |
|---|---|---|
...
Sau vài tháng dùng thử, doanh nghiệp vượt quá 50 giao dịch/tháng của gói Free. Đây là lúc trả phí cho Klassi AI — **khác hoàn toàn** với giao dịch nghiệp vụ của doanh nghiệp ở Giai đoạn 3.
...
Bấm "Nâng cấp lên Pro — 799.000đ/tháng (2.000 giao dịch)"
        │
        ▼
Klassi AI tạo Payment Link qua PayOS
(đây là PayOS account CỦA KLASSI AI, không phải của doanh nghiệp)
        │
        ▼
...
PayOS gửi webhook callback về Klassi AI
        │
        ▼
Klassi AI tự động kích hoạt gói Pro, reset quota
        │
        ▼
Admin thấy thông báo "Nâng cấp thành công!" — tiếp tục dùng bình thường
```

> **Phân biệt quan trọng:** Giai đoạn 3 — AI phân loại giao dịch ngân hàng của doanh nghiệp. Giai đoạn 4 — doanh nghiệp trả phí dịch vụ cho Klassi AI qua PayOS. Hai luồng tiền hoàn toàn tách biệt.
...
| **Cas** | Hạ tầng nền — đọc biến động số dư ngân hàng, tự động bắn webhook về Klassi AI | Chạy ngầm, không ai thấy |
| **Klassi AI** | Lớp ứng dụng trung gian — nhận giao dịch từ Cas, dùng AI định khoản TT133, hiển thị cho kế toán xác nhận | Real-time |
| **PayOS** | Cổng thanh toán cho riêng việc thu phí dịch vụ (Klassi AI là bên bán, doanh nghiệp là bên mua gói) | Khi doanh nghiệp nâng cấp/gia hạn gói |
| **Cas Partner** (nhân sự Cas/CASSO) | Theo dõi tổng quan các doanh nghiệp đang dùng Klassi AI, xem doanh thu, khóa/mở tài khoản vi phạm, hỗ trợ kỹ thuật | Khi cần hỗ trợ hoặc theo dõi vận hành |

---

## 💡 Điểm mấu chốt cần nhớ

> Với khách hàng cuối (chủ doanh nghiệp, kế toán), **Cas gần như "vô hình"** sau bước Onboarding. Họ chỉ cảm nhận mình đang dùng Klassi AI — không thấy được lớp hạ tầng Cas phía sau.

Kiến trúc tách bạch 2 cơ chế (đăng nhập Klassi AI ≠ liên kết ngân hàng qua Cas) là đúng đắn — người dùng không cần hiểu hay quan tâm đến Cas, họ chỉ cần trải nghiệm mượt mà trên Klassi AI.

> **Hai dòng tiền hoàn toàn tách biệt, đừng nhầm lẫn:**
> - **Dòng tiền nghiệp vụ** (Giai đoạn 3): giao dịch ngân hàng của doanh nghiệp → Klassi AI đọc + định khoản TT133.
> - **Dòng tiền dịch vụ** (Giai đoạn 4): doanh nghiệp → Klassi AI, qua PayOS, để trả phí sử dụng phần mềm.
>
> Klassi AI không bao giờ "chạm" vào dòng tiền nghiệp vụ — chỉ đọc dữ liệu giao dịch qua Cas để định khoản, không giữ tiền hộ ai.
